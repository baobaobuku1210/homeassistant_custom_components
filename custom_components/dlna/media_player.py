...
from homeassistant.components.media_player.const import (
    ...
    MEDIA_TYPE_TVSHOW, MEDIA_TYPE_VIDEO, MEDIA_TYPE_PLAYLIST)
...
class DLNADevice(object):
    ...
    def Dlna_name(self):
        location_str=self.location.replace('http://','').replace('https://','')
        return location_str[0:location_str.find(':')].replace('.','_')
...
class DLNAUPNPDevice:
    ...
    def __init__(self, devElem, devNs, rootDevice):
        ...
        if serviceList is not None:
            for service in list(serviceList):
